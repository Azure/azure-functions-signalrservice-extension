...
namespace SignalRServiceExtension.Tests
{
    public class SignalRTriggerTests
    {
        ...
        public async Task CreateListenerTest()
        {
            ...
            var binding = new SignalRTriggerBinding(parameterInfo, new SignalRTriggerAttribute{HubName = hub, Target = method}, dispatcher);
            await binding.CreateListenerAsync(listenerFactoryContext);
            Assert.Equal(executor, dispatcher.Executors[(hub, method)]);
        }
